...
def load_model(model, path):
    ...
    # Load state dict safely with weights_only=True to prevent arbitrary code execution
    # and map_location for device compatibility
    state = torch.load(path_model, map_location='cpu', weights_only=True)
    # Use strict=False to handle architecture variations (e.g., ResNet18 with custom FC layer)
