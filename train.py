...
def main(args):
   ...
   # Set the device (cuda, cpu or mps)
    ...
    # Track the best test loss seen so far (lower is better)
    best_loss = np.inf
    ...
    for epoch in pbar:
        ...
        # Save best model based on the test loss (save only when it improves)
        # Use the most recent `loss` value returned by `test_model` for this epoch.
        if loss < best_loss:
            best_loss = loss
